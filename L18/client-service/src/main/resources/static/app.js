...
const connect = () => {
  ...
  stompClient.connect({}, (frame) => {
    ...
    stompClient.subscribe(topicName, (message) => showMessage(JSON.parse(message.body).text));
    stompClient.subscribe(topicNameUser, (message) => showMessage(JSON.parse(message.body).text));
  });
}
...
const sendMsg = () => {
  ...
  stompClient.send(`/app/message.${roomId}`, {}, JSON.stringify({'text': message}))
}
